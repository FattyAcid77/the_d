...
func _load_defs() -> void:
	...
	if not ResList.dir_exists(DEFS_DIR):
		push_warning("PopupWindows: no Defs folder at %s" % DEFS_DIR)
		return
	for path in ResList.tres_files(DEFS_DIR):
		var r := load(path)
...
## Adds the window to the tree and places it, one frame later
func _attach(w: PopupWindow, d: PopupWindowDef) -> void:
	...
	w.remember_position()
	# The portal's follow was armed while the window was still at 0,0
...
func _player_screen_position() -> Vector2i:
	# get_first_node_in_group() returns a plain Node
...
## "Behind the game" - Godot has always_on_top but no always_on_bottom
...
## Called by a window that has morphed into a different def
...
## The one icon the operating system shows for this program - taskbar, alt-tab
